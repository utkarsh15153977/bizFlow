...
export function NotificationsView({ notifications }: { notifications: Notification[] }) {
  ...
  const filtered = useMemo(() => notifications.filter((notification) =>
    (readFilter === "all" || (readFilter === "unread" && !notification.is_read) || (readFilter === "read" && notification.is_read)) &&
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="flex flex-wrap gap-3">
          <label className="space-y-1.5 text-sm font-medium">State<select value={readFilter} onChange={(event) => { setReadFilter(event.target.value); setPage(1); }} className="block rounded-lg border border-border bg-background px-3 py-2 text-sm font-normal"><option value="all">All</option><option value="unread">Unread</option><option value="read">Read</option></select></label>
          <label className="space-y-1.5 text-sm font-medium">Type<select value={typeFilter} onChange={(event) => { setTypeFilter(event.target.value); setPage(1); }} className="block rounded-lg border border-border bg-background px-3 py-2 text-sm font-normal"><option value="all">All types</option>{notificationTypes.map((type) => <option key={type}>{type}</option>)}</select></label>
        </div>
      ...
      {visible.length === 0 ? <EmptyState title="No notifications" description="You are all caught up." /> : <div className="space-y-3">{visible.map((notification) => {
      ...
      {filtered.length > pageSize && <div className="flex items-center justify-between text-sm text-muted-foreground"><span>Page {page} of {pageCount}</span><div className="flex gap-2"><Button variant="secondary" disabled={page === 1} onClick={() => setPage((current) => current - 1)}>Previous</Button><Button variant="secondary" disabled={page === pageCount} onClick={() => setPage((current) => current + 1)}>Next</Button></div></div>}
    </div>
  );
}
